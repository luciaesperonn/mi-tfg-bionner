"""
Barrido fino con metricas extra para graficas de precision/recall:
  - macro_precision / macro_recall (mismo promedio "media de por-relacion" que
    ya usa macro_f1 en score.py) -- para explicar por que se hunde el F1 cerca
    del borde del grid.
  - precision/recall/FPR "binarios" (existe relacion vs no_relation, ignorando
    el tipo exacto) -- para la curva PR y la curva ROC.

Reusa las probabilidades ya cacheadas (blind_probs_{exp}.npy), igual que
fine_resweep.py y fine_resweep_curvas.py.

Uso: python baseline/fine_resweep_pr.py  (desde la raiz del repo o desde baseline/)
"""
import json
import sys
...
def main():
    ...
    for cfg in MULTISEED_CONFIGS:
        for seed in SEEDS:
            ...
            if not npy_path.exists():
                ...
                continue
            print(f"[pr] {cfg['exp']} seed={seed} ...", flush=True)
    ...
    out_path = out_dir / "multiseed_finegrid_pr.csv"
    df.to_csv(out_path, index=False)
    print(f"Guardado: {out_path}  ({len(df)} filas)  "
          f"[total_candidates={TOTAL_CANDIDATES}, total_positivos={TOTAL_POSITIVES}]")
